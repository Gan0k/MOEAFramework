...
public class GeneralizedSpread extends NormalizedIndicator {
	 ...
	 * approximation and reference sets should be normalized. Returns {@code
	 * Double.POSITIVE_INFINITY} if the approximation set is empty.
	 *
	 * @param problem the problem
	...
	static double evaluate(Problem problem,
		...
		int numberOfObjectives =
			approximationSet.get(0).getNumberOfObjectives();
		...
		if (IndicatorUtils.euclideanDistance(problem,approximationSet.get(0),
			...
			for (int i = 0; i < approximationSet.size(); i++) {
				distances[i] =
					IndicatorUtils.distanceToNearestNeighbour(problem, i,
							approximationSet);
			}
